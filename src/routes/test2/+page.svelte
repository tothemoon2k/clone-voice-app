...
{#if step === 0}
    <Landing>
        <button on:click={handleNext} class="justify-self-end bg-blue-500 w-5/6 py-2.5 font-medium text-white rounded-xl">Get Started</button>
    </Landing>
{:else if step === 1}
...
{:else if step === 5}
    <Step5 bind:voiceRecording={voiceRecording} bind:loading={step5Loading}>
        <button on:click={handleNext} disabled={step5Loading} class="disabled:cursor-not-allowed justify-self-end bg-blue-500 w-5/6 py-2.5 font-medium text-white rounded-xl">
            {#if !step5Loading}
                Continue
            {:else}
                <div role="status">
...
{:else if step === 6}
    <Step6 bind:firstMessage={firstMessage}>
        <button on:click={handleCall} class="justify-self-end bg-blue-500 w-5/6 py-2.5 font-medium text-white rounded-xl">
            {#if !step6Loading}
                Call
            {:else}
                <div role="status">
